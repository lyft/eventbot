...
# We need to set EVENTBOT_DOCS_VERSION_STRING and EVENTBOT_DOCS_RELEASE_LEVEL for Sphinx.
...
then
  ...
  export EVENTBOT_DOCS_VERSION_STRING="tag-$TRAVIS_TAG"
  export EVENTBOT_DOCS_RELEASE_LEVEL=tagged
  export EVENTBOT_BLOB_SHA="$TRAVIS_TAG"
else
  BUILD_SHA=$(git rev-parse HEAD)
  VERSION_NUM=$(cat VERSION)
  export EVENTBOT_DOCS_VERSION_STRING="${VERSION_NUM}"-"${BUILD_SHA:0:6}"
  export EVENTBOT_DOCS_RELEASE_LEVEL=pre-release
  export EVENTBOT_BLOB_SHA="$BUILD_SHA"
fi
...
set -x
sphinx-apidoc -o "${GENERATED_AUTOGEN_RST_DIR}" -T -P ../eventbot
