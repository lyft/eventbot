...
#   https://lyft.github.io/eventbot/docs/v1.6.0/.
# * Master commit: pushes docs to https://lyft.github.io/eventbot/docs/latest/.
# * Otherwise: noop.
#
...
CHECKOUT_DIR=../eventbot-docs
BUILD_SHA=`git rev-parse HEAD`

if [ -n "$TRAVIS_TAG" ]
then
  PUBLISH_DIR="$CHECKOUT_DIR"/docs/eventbot/"$TRAVIS_TAG"
elif [ -z "$TRAVIS_PULL_REQUEST" ] && [ "$TRAVIS_BRANCH" == "master" ]
then
  PUBLISH_DIR="$CHECKOUT_DIR"/docs/eventbot/latest
else
...
git add .
echo 'commit'
git commit -m "docs eventbot@$BUILD_SHA"
echo 'push'
